...
<body>
    <header class="header">
        <h1>🤖 X-Monitor</h1>
        <div class="meta">Twitter热点监控 | {date} 更新</div>
    </header>
    <nav class="nav">
        <div class="nav-links">
            {nav_links}
...
class WebGenerator:
    ...
    def generate(self, data, category_config):
        """生成HTML"""
        ...
        for key, config in category_config.items():
            tweets = data.get(key, [])
            categories += self._generate_category(key, config['name'], tweets)
    ...
    def _generate_category(self, key, name, tweets):
        """生成单个分类"""
...
if __name__ == '__main__':
    # 测试
    gen = WebGenerator()
    gen.generate({'test': [{'user': '@test', 'content': 'Test', 'date': 'now', 'url': ''}]}, {'test': {'name': 'Test'}})
